...
export default function Store({ paymentProcessor, dai }:any) {
    const buy = async (item:any) =>{
        // const response1fetch = await fetch(`${API_URL}/api/getPaymentId/${item.id}`);
        // const response1 = await response1fetch.json()
        ...
        // const response2fetch = await fetch(`${API_URL}/api/getItemUrl/${response1.data.paymentId}`);
        // const response2 = await response2fetch.json()
